  ...
  <thead>
    ...
    @foreach ($reports_this_week as $report)
      <tr>
        ...
        <td><div class="col">
          <input type="text" name="this_week_task_name[]" 
          class="form-control-plaintext mt-0 pt-0 border border-secondary" value="{{$report->task_name}}" placeholder="入力してください"></input>
          </div>
        </td>
        <td>
          <div class="col">
          <textarea class="form-control border border-secondary" name="this_week_task_detail[]" placeholder="入力してください">{{$report->task_detail}}</textarea >
          </div>
        </td>
        <td>
          <div class="d-flex"">
            <div class="col">
              <input type="text" name="this_week_m[]" 
...
<div class="col-5"><textarea cols="65" rows="6" placeholder="状況報告"></textarea></div>
  <div class="col-5 d-flex justify-content-center">
    <div class="col m-3">
      <div class="row">今週の予定工数(M)</div>
      <div class="row">
        <span class="card" class="border border-secondary" 
        style="padding:4px 0px 12px 0px; width:120px; line-height:68px;font-size:32px;text-align: center;">
        240
        </span>
      </div>
    </div>
  ...
  <div class="col">
    ...
    <div class="row py-1"><button type="submit" formaction="{{ url('/report_daily/csvimport')}}" name="btn_import_this_week_csvfile" class="btn btn-outline-secondary disabled">読込（日報データ）</button></div>
    <div class="row py-1"><input type="file" name="file_this_week_csv" accept=".csv, .CSV"></input></div>
  </div>
</div>
