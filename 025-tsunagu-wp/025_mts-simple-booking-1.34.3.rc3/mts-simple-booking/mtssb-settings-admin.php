...
<?php
	 ...
	 *
	 */
    public function controlsFieldsSetting()
    {
        $option_name = sprintf('%s_%s', $this->domain, $this->tab);
	...
	public function add_fields_settings() {
		...
		if ($this->tab == 'controls') {
            ...
            add_settings_field('block_mail', __('Block Booking Mail', $this->domain), array($this, 'reserve_form'),
                $option_name, $option_name, array('label_for' => 'block_mail'));
            ...
            add_settings_field('user_consent', __('User Consent', $this->domain), array($this, 'miscellaneous_form'),
                $option_name, $option_name, array('label_for' => 'user_consent'));
            add_settings_field('user_url', __('Terms Of User', $this->domain), array($this, 'miscellaneous_form'),
                $option_name, $option_name, array('label_for' => 'user_url'));
            add_settings_field('user_mail', __('Registration Mail', $this->domain), array($this, 'miscellaneous_form'),
                $option_name, $option_name, array('label_for' => 'user_mail'));
			...
			case 'awaking' : ?>
				...
				<span id="awaking-time-area"><?php _e('Every hour', $this->domain) ?>
					...
					<span id="awaking-crontab-area">
                        ...
                        <input type="hidden" name="mts_simple_booking_controls[awaking][log]" value="0">
					</span>
				</span><br>
	...
	public function miscellaneous_form($args) {
		switch ($args['label_for']) {
			...
			case 'schedule_dialog' : ?>
				...
				break;
            case 'user_consent': ?>
                <label id="user_consent_off"><input type="radio" name="mts_simple_booking_miscellaneous[user_consent]" value="0"<?php echo empty($this->data['user_consent']) ? ' checked="checked"' : '' ?> /><?php _e('Inactive', $this->domain) ?></label>&nbsp;
                <label id="user_consent_on"><input type="radio" name="mts_simple_booking_miscellaneous[user_consent]" value="1"<?php echo $this->data['user_consent'] ? ' checked="checked"' : '' ?> /><?php _e('Active', $this->domain) ?></label><br />
                <?php _e('Demand agreement check of the user who read the terms of use.', $this->domain);
                break;
            case 'user_url': ?>
                <input id="user_url" type="text" name="mts_simple_booking_miscellaneous[user_url]" value="<?php echo esc_html($this->data['user_url']) ?>" style="width:80%" />
                <?php
                break;
            case 'user_mail':
                ...
                <select id="user_registration_male" class="mts-select" name="mts_simple_booking_miscellaneous[user_mail]">
                    <?php foreach ($lists as $option_name => $mail_subject) {
                        $selected = $this->data['user_mail'] == $option_name ? ' selected="selected"' : '';
                        echo sprintf('<option value="%s"%s>%s</option>', $option_name, $selected, $mail_subject);
                    } ?>
                </select><br>
                <?php _e('Choose a template of user registration mail from the box above.', $this->domain);
                break;
			default :
				break;
		}
	...
	public function charge_form($args) {
		switch ($args['label_for']) {
			...
			case 'consumption_tax' : ?>
				...
				break;
			case 'pay_first' : ?>
                <label><input type="radio" name="mts_simple_booking_charge[pay_first]" value="0"<?php echo $this->data['pay_first'] == 0 ? ' checked="checked"' : '' ?> /> <?php _e('Inactive', $this->domain) ?></label>
                <label><input type="radio" name="mts_simple_booking_charge[pay_first]" value="1"<?php echo $this->data['pay_first'] == 1 ? ' checked="checked"' : '' ?> /> <?php _e('Active', $this->domain) ?></label>
                <label><input type="radio" name="mts_simple_booking_charge[pay_first]" value="2"<?php echo $this->data['pay_first'] == 2 ? ' checked="checked"' : '' ?> /> <?php _e('Zero Acceptable', $this->domain) ?></label><br />
	...
	public function reserve_form($args) {
		switch ($args['label_for']) {
            case 'block_mail' : ?>
                <input type="hidden" id="booking_mail_" name="mts_simple_booking_reserve[block_mail]" value="0" />
                <input id="booking_mail" name="mts_simple_booking_reserve[block_mail]" value="1" type="checkbox"<?php echo $this->data['block_mail'] ? ' checked="checked"' : '' ?> /><br />
                <?php _e('Check if the system does not send a booking mail to the customer.', $this->domain);
                break;
			case 'column' :
	...
	private function _get_default($tab) {
		...
		$options = array(
			...
			'miscellaneous' => array(
                ...
                'user_consent' => 0,    // ユーザー登録の同意設定
                'user_url' => '',       // ユーザーの利用規約URL
                'user_mail' => '',      // ユーザー登録メールのテンプレート名
			),
			...
			'reserve' => array(
                'block_mail' => 0,      // 0:予約メール送信 1:送信しない
				...
				'column_order' => 'company,name,furigana,email,postcode,address,tel,birthday,gender,newuse',
				'title' => '【ご予約を承りました】',
