 ...
 * @Author		S.Hayashi
 *
 * Updated to 1.34.3 on 2021-06-24
 * Updated to 1.34.2 on 2021-05-18
 * Updated to 1.34.1 on 2021-02-19
...
if (!class_exists('MTSSB_Booking')) {
    require_once('mtssb-booking.php');
}
if (!class_exists('MtssbCalendar')) {
    require_once('lib/MtssbCalendar.php');
}
...
class MTSSB_Booking_Form extends MTSSB_Booking
{
	...
	public function check_post_booking() {
		...
		if (!$this->_input_validation($check_mail2)) {
            ...
            $this->err_message .= '<br>' . implode('<br>', $this->errmsg);
			return false;
		}
	...
	protected function _input_validation($check_mail2=true) {
		...
		if (1 == $clcols['birthday']) {
		    if (!$this->_check_age_limit($this->calendar, $this->booking['client']['birthday'])) {
	...
	<table>
		...
		<tr class="booking-form-people-number-row">
			...
			<td><?php
				...
				<?php if ($this->charge['charge_list'] == 1 && !empty($prices)) {
...
<?php
	...
	{
		global $usces, $mts_simple_booking;
		...
		// ログイン中であればログインユーザー情報をセットする
		    ...
		    $oUser = $mts_simple_booking->oUser;
            $client['company'] = $oUser->company;
            $client['sei'] = $oUser->sei;
            $client['mei'] = $oUser->mei;
            $client['sei_kana'] = $oUser->sei_kana;
            $client['mei_kana'] = $oUser->mei_kana;
            $client['name'] = $oUser->name;
            $client['furigana'] = $oUser->furigana;
            $client['email'] = $oUser->email;
            $client['postcode'] = $oUser->postcode;
            $client['address1'] = $oUser->address1;
            $client['address2'] = $oUser->address2;
            $client['tel'] = $oUser->tel;
            $client['birthday'] = $oUser->birthday; // new MTS_WPDate,
            $client['gender'] = $oUser->gender;
            $this->booking['user_id'] = $oUser->oWPUser->ID;
		}
...
?>
	...
	<?php foreach ($column_order as $column) : $column_use = $this->reserve['column'][$column]; if (0 < $column_use) :
		...
		case 'birthday' : ?><tr>
			...
			<td>
            <?php echo $this->oHelper->selectBirthday('form_birthday', 'booking-date', 'booking[client][birthday]', $client['birthday']);
			break;
		case 'gender' : ?><tr>
			...
			<?php echo $this->oHelper->radioButton('client-gender', 'content-text radio', 'booking[client][gender]', MTSSB_User::$gender, $client['gender']);
            break;
		case 'email' : ?><tr>
...
<?php
	...
	private function _out_option_price($option)
	{
		...
		echo sprintf('<div class="unit-price %s %s">%s</div>', $option->type, $option->keyname, $price);
	}
...
?>
		<tr>
			...
			<td><?php foreach ($option->field as $fieldname => $fielda) :
                // 選択肢ラベルの後方に料金を出力する
                $priceSeg = '';
                if ($this->charge['charge_list'] == 1 && is_numeric($fielda['price'])) {
                    $price = apply_filters("booking_form_option_price", $this->money_format($fielda['price']), $option->keyname);
                    if (!empty($price)) {
                        $priceSeg = sprintf(' <span class="option-radio-price %s">%s</span>', $fieldname, $price);
                    }
                } ?>
                <label class="field-item check-<?php echo $option->keyname ?>">
					<input id="option-radio-<?php echo "{$option->keyname}-{$fieldname}" ?>" type="radio" name="booking[options][<?php echo $option->keyname ?>]" value="<?php echo $fieldname ?>"<?php echo $fieldname == $val ? ' checked="checked"' : '' ?> /> <?php
                        echo sprintf('%s%s', $fielda['label'], $priceSeg); ?>
				</label>
			<?php endforeach; ?>
				<?php
					$this->_out_option_note($option); // 注記
			...
			<td>
				...
				<?php foreach ($option->field as $fieldname => $fielda) :
                    ...
                    if ($this->charge['charge_list'] == 1 && is_numeric($fielda['price'])) {
                        $price = apply_filters("booking_form_option_price", $this->money_format($fielda['price']), $option->keyname);
                        if (!empty($price)) {
                            $priceSeg = sprintf(' <span class="option-check-price %s">%s</span>', $fieldname, $price);
                        }
                    } ?>
					<option value="<?php echo $fieldname ?>"<?php echo $fieldname == $val ? ' selected="selected"' : '' ?>><?php
                        echo sprintf('%s%s', $fielda['label'], $priceSeg); ?></option>
		...
		<tr>
			...
			<td>
                <?php foreach ($option->field as $fieldname => $fielda) :
					...
					<label class="field-item check-<?php echo $option->keyname ?>">
						<input id="option-check-<?php echo "{$option->keyname}-{$fieldname}" ?>" type="checkbox" name="booking[options]<?php echo "[$option->keyname][$fieldname]" ?>" value="1"<?php echo $option->isChecked($fieldname) ? ' checked="checked"' : '' ?> /> <?php
                            echo sprintf('%s%s', $fielda['label'], $priceSeg); ?>
					</label>
				<?php endforeach;
					$this->_out_option_note($option); // 注記
...
<?php
	...
	private function _outform_option_date($option)
	{
        ...
        // 日付入力ボックスの入力年の範囲指定
        $startYear = apply_filters('booking_form_option_date_start', date_i18n('Y', $this->booking['booking_time']), $option->keyname, $param);
        $endYear = apply_filters('booking_form_option_date_end', $startYear + 2, $option->keyname, $param);
        $dateVal = 0 < $option->getValue() ? date_i18n('Y-m-d', $option->getValue()) : '0-0-0';
?>
		<tr>
			<th><?php // ラベル表示
				$this->_out_option_label_header($option, false); ?>
			...
			<td>
				<?php
                    echo $this->oHelper->selectDate("booking-option-{$option->keyname}",
                        "booking-option-date", "booking[options][{$option->keyname}]", $dateVal, $startYear, $endYear);
					$this->_out_option_note($option); // 注記
					$this->_out_option_price($option); // 単価
					$this->_out_option_err_message($option->keyname); // エラー
				?>
...
<?php
	...
	private function _outconfirm_button()
	{
        ...
        // 予約ボタン出力の確認
        $booking_out = $this->charge['pay_first'] == 0;
        
        // PayPalボタン出力の確認
        $paypal_out = 0 < $this->charge['checkout'];

        if ($paypal_out && $this->bill->get_total() == 0) {
            $paypal_out = false;
            if ($this->charge['pay_first'] == 2) {
                $booking_out = true;
            }
        }

        $paypal_out = apply_filters('mtssb_paypal_button_out', $paypal_out, $param);
        $booking_out = apply_filters('mtssb_booking_button_out', $booking_out, $param);

		// ボタン出力前のメッセージ
		$button_message = '';
		if ($paypal_out && $booking_out) {
			$button_message = '銀行振込などその他の場合は「予約する」を押してご予約下さい。';
		} elseif ($paypal_out) {
...
?>
	...
	<?php foreach ($column_order as $column) : if (0 < $this->reserve['column'][$column]) :
		...
		case 'birthday' : ?><tr>
			...
			<td><?php //$oDate = date_create($client['birthday']);
			    //echo $oDate->format('Y年n月j日');
                $ymd = explode('-', $client['birthday']);
                echo sprintf('%s年%s月%s日', ($ymd[0] == 0 ? ' ' : $ymd[0]), ($ymd[1] == 0 ? ' ' : sprintf('%02d', $ymd[1])), $ymd[2] == 0 ? ' ' : sprintf('%02d', $ymd[2]));
                //echo sprintf('<input type="hidden" name="booking[client][birthday][year]" value="%d">', $oDate->format('Y'));
                //echo sprintf('<input type="hidden" name="booking[client][birthday][month]" value="%d">', $oDate->format('n'));
                //echo sprintf('<input type="hidden" name="booking[client][birthday][day]" value="%d">', $oDate->format('j'));
                echo sprintf('<input type="hidden" name="booking[client][birthday][year]" value="%d">', $ymd[0]);
                echo sprintf('<input type="hidden" name="booking[client][birthday][month]" value="%d">', $ymd[1]);
                echo sprintf('<input type="hidden" name="booking[client][birthday][day]" value="%d">', $ymd[2]); ?>
			</td>
			<?php break;
		case 'gender' : ?><tr>
			...
			<td class="option-confirm-value">
				...
				<?php switch ($option->getType()) :
					...
					case 'date':
                        ...
                        <input type="hidden" name="booking[options][<?php echo $option->keyname ?>][year]" value="<?php echo $yy ?>">
                        <input type="hidden" name="booking[options][<?php echo $option->keyname ?>][month]" value="<?php echo $mm ?>">
                        <input type="hidden" name="booking[options][<?php echo $option->keyname ?>][day]" value="<?php echo $dd ?>">
						<?php break;
					case 'time':
						$otime = new MTS_WPTime($option->getValue());
