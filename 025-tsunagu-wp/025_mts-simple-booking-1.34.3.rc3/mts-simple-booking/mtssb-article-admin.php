...
<?php
	...
	public function meta_box_miscellaneous($post)
	{
        ...
        // メールのテンプレート名リストを取得する
        $templates = MTSSB_Mail_Template::listTemplates();

        // 予約完了メールに設定したテンプレート名
        $confirmationMail = $article['addition']->template;
        if (preg_match('/^[0-9]+$/', $article['addition']->template)) {
            $confirmationMail = MTS_Simple_Booking::DOMAIN . MTSSB_Mail_Template::TEMPLATE . $article['addition']->template;
        }

        // 予約通知メールに設定したテンプレート名
        $awakingMail = $article['addition']->awaking_mail;
        if (preg_match('/^[0-9]+$/', $article['addition']->awaking_mail)) {
            $awakingMail = MTS_Simple_Booking::DOMAIN . MTSSB_Mail_Template::TEMPLATE . $article['addition']->awaking_mail;
        }
...
?>
	...
	<table class="form-table">
		...
		<tr>
			<th scope="row"><label for="article-addition-option_name"><?php _e('Option group', $this->domain) ?></label></th>
			<td>
        ...
        <tr>
            ...
            <td>
                <select id="article-addition-template" name="article[addition][template]" style="letter-spacing:1px">
                    <?php foreach ($templates as $option_name => $templateName) {
                        echo sprintf('<option value="%s"%s>%s</option>', $option_name, ($option_name == $confirmationMail ? ' selected="selected"' : ''), $templateName);
                    } ?></select><br />
		...
		<tr>
			...
			<td>
				...
				<select id="article-addition-awaking_mail" name="article[addition][awaking_mail]" style="letter-spacing:1px">
					<?php foreach ($templates as $option_name => $templateName) {
                        echo sprintf('<option value="%s"%s>%s</option>', $option_name, ($option_name == $awakingMail ? ' selected="selected"' : ''), $templateName);
                    } ?></select><br>
