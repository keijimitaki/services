 ...
 * Updated to 1.34.2 on 2021-05-20
 * Updated to 1.34.1 on 2021-03-20
 * Updated to 1.34.0 on 2020-08-26
...
class MTS_Simple_Booking
{
    ...
    // ウィジェットを準備する
    private function _setupWidget()
    {
        // 予約カレンダーウィジェット
        if (!class_exists('MTSSB_Calendar_Widget')) {
        ...
        // ミックスカレンダーウィジェット
        if (!class_exists('MTSSB_Dailylink_Widget')) {
    ...
    {
        if (!class_exists('MTSSB_User')) {
            require_once('mtssb-user.php');
        }

        $this->oUser = new MTSSB_User;

        $this->oUser->oWPUser = wp_get_current_user();
    ...
    {
        // 管理画面ユーザープロフィール編集
        add_action('load-profile.php', [$this, 'loadAdminUser']);
        add_action('load-user-edit.php', [$this, 'loadAdminUser']);
        add_action('load-user-new.php', [$this, 'loadAdminUser']);
	...
	public function add_admin_menu()
    {
		...
		$caledarHook = add_submenu_page(self::ADMIN_MENU, __('Calendar', self::DOMAIN), __('Calendar', self::DOMAIN), 'administrator', self::ADMIN_MENU, array($this, 'menu_calendar'));
        add_action('load-' . $caledarHook, array($this, 'loadAdminCalendar'));

        $listHook = add_submenu_page(self::ADMIN_MENU, __('List Booking', self::DOMAIN), __('List Booking', self::DOMAIN), 'administrator', self::PAGE_LIST, array($this, 'menu_list'));
        add_action('load-' . $listHook, array($this, 'loadAdminList'));

		$bookingHook = add_submenu_page(self::ADMIN_MENU, __('Add & Edit', self::DOMAIN), __('Add & Edit', self::DOMAIN), 'administrator', self::PAGE_BOOKING, array($this, 'menu_booking'));
        add_action('load-' . $bookingHook, array($this, 'loadAdminBooking'));

		$scheduleHook = add_submenu_page(self::ADMIN_MENU, __('Schedule', self::DOMAIN), __('Schedule', self::DOMAIN), 'administrator', self::PAGE_SCHEDULE, array($this, 'menu_schedule'));
		add_action('load-' . $scheduleHook, array($this, 'loadAdminSchedule'));

		$optionHook = add_submenu_page(self::ADMIN_MENU, __('Option', self::DOMAIN), __('Option', self::DOMAIN), 'administrator', self::PAGE_OPTION, array($this, 'menu_option'));
		add_action('load-' . $optionHook, array($this, 'loadAdminOption'));

        $templateHook = add_submenu_page(self::ADMIN_MENU, __('Mail Template', self::DOMAIN), __('Mail Template', self::DOMAIN), 'administrator', self::PAGE_MAIL_TEMPLATE, array($this, 'menu_mail_template'));
        add_action('load-' . $templateHook, array($this, 'loadAdminTemplate'));
	 ...
	 * 管理画面メニュー処理　オプション
	 */
    public function loadAdminOption()
    {
    ...
    {
        require_once('mtssb-user-admin.php');
        $this->oUserAdmin = new MTSSB_User_Admin;
        // ユーザー管理パックが組み込まれていなければユーザー項目を追加する
        //if (!$this->mtscu_activation) {
        //    add_filter('user_contactmethods', array($this->oUserAdmin, 'extend_user_contactmethod'));
        //}
    }
	...
	public function form_dispatcher($content) {
        ...
        // cronメンテナンス
        elseif (is_page('mtssb-cron')) {
		    $content = $this->cronMaintenance();
        }
    ...
    // cronメンテナンス
    private function cronMaintenance()
    {
        $post = stripslashes_deep($_POST);
        $res = '';

        if (isset($post['nonce']) && wp_verify_nonce($post['nonce'], 'mtssb-cron')) {
            if (isset($post['time_stamp'])) {
                $res = sprintf('UTC:%s<br>JST:%s', time(), current_time('timestamp'));
            }
            elseif (isset($post['next_scheduled'])) {
                $res = wp_next_scheduled(self::CRON_AWAKING);
        ...
        $content = $this->cronMaintenanceView() . sprintf('<div class="mtssb-command-result">%s</div>', $res);

        return $content;
    }

    // cronメンテナンスビュー
    private function cronMaintenanceView()
    {
        ob_start();
?>
        <form method="post">
            <div class="mtssb-command">
                <input type="submit" name="time_stamp" value="Unix Time Stamp">
            </div>
            <div class="mtssb-command">
                <input type="submit" name="next_scheduled" value="wp_next_scheduled">
            </div>
            <input type="hidden" name="nonce" value="<?php echo wp_create_nonce('mtssb-cron') ?>">
        </form>
...
<?php
    ...
    static public function getSessionId()
    {
        ...
        // 重複しないsessionIdを生成する(重複の場合5回トライする)
        else {
            for ($i = 0; $i < 5; $i++) {
                $sessionId = md5(uniqid(mt_rand(), true));
                $data = get_transient(self::SESSION_NAME . $sessionId);
                if (empty($data)) {
                    // COOKIEにIDを保存する
        ...
        // セッションIDのチェック
        if (strlen($sessionId) === 32 && preg_match('/^[0-9a-f]*\z/', $sessionId)) {
            return $sessionId;
        }
