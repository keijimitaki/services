...
<?php
	...
	protected function _confirmation_form()
	{
        ...
        $email = $this->search['client_email'];
        if (empty($email) && is_user_logged_in()) {
            $email = $mts_simple_booking->oUser->oWPUser->get('user_email');
        }
...
?>
	...
	<?php foreach ($column_order as $column) : if (0 < $this->reserve['column'][$column]) :
		...
		case 'birthday' : ?><tr>
			...
			<td><?php $ymd = explode('-', $client['birthday']);
                $birthday = sprintf('%s年%s月%s日', ($ymd[0] == 0 ? ' ' : $ymd[0]), ($ymd[1] == 0 ? ' ' : sprintf('%02d', $ymd[1])), $ymd[2] == 0 ? ' ' : sprintf('%02d', $ymd[2]));
                echo apply_filters('booking_form_birthday_date', $birthday, $client['birthday']) ?>
			</td>
			<?php break;
		case 'gender' : ?><tr>
