...
<?php
    ...
    {
        $bookingDate = date_i18n('Y-m-d', $this->booking['booking_time']);
        $startYear = date_i18n('Y') - 1;
        $endYear = $startYear + 4;
			...
			<table class="form-table" style="width: 100%">
				<tr class="form-field">
					...
					<td>
						...
						<?php echo $this->oHelper->selectDate('booking_time', 'booking-date', 'booking', $bookingDate, $startYear, $endYear) ?>
		...
		<div class="inside">
			...
			<?php for ($i = 0; $i < $opt_number; $i++) : $option = $this->booking['options'][$i]; ?><tr>
				...
				<td><?php switch ($option->getType()) :
					...
					case 'date':
					    $val = $option->getValue();
                        $yy = date_i18n('Y', empty($val) ? current_time('timestamp') : $val);
                        $val = empty($val) ? '' : date_i18n('Y-n-j', $val);
					    echo $this->oHelper->selectDate("booking-options-{$option->getKeyname()}", 'option-date-box', "booking[options][{$option->getKeyname()}]", $val, $yy + 1, $yy - 1);
                        echo ' (0 | ' . $option->getPrice() . ')';
						break;
					case 'time':
		...
		<div class="inside">
			<table class="form-table" style="width: 100%">
				...
				<tr>
					...
					<td><?php
                            echo $this->oHelper->selectBirthday('booking-birthday', 'booking-date', 'booking[client][birthday]', $client['birthday']);
                        ?>
					</td>
				</tr>
				<tr>
		...
		<form id="select-article" method="get" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
			...
			<table class="form-table" style="width: 100%">
				<tr class="form-field">
					...
					<td>
						<?php echo $this->oHelper->selectDate('booking_new', 'booking-date', 'bd', $bookingDay, $y - 1, $y + 3) ?>
					</td>
				</tr>
				<tr class="form-field">
					<th>
