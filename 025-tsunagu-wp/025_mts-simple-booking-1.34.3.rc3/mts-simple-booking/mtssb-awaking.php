 ...
 * @Author		S.Hayashi
 *
 * Updated to Ver.1.34.0 on 2020-08-22
...
class MTSSB_Awaking
{
    ...
    private function _awakingBooking($article)
    {
        ...
        if (empty($bookings)) {
            $this->_errorFileOut("ID:{$article['article_id']}, No Data");
            return;
        }

        // 事前メールのテンプレートを取得する
        if (!$this->oMailTemplate->getTemplate($article['addition']->awaking_mail)) {
            $this->_errorFileOut("ID:{$article['article_id']}, No Template");
            return;
        }
        ...
        foreach ($bookings as $booking) {
            ...
            if ($this->_sendMail($booking['booking_id'], $this->oMailTemplate, $article)) {
    ...
    // エラー出力動作
    private function _errorFileOut($msg='Error')
    {
        if (!isset($this->controls['awaking']['log']) || $this->controls['awaking']['log'] <= 0) {
            return;
        }

        $tfp = fopen(__DIR__ . '/awaking_error.txt', 'a');
        fputs($tfp, sprintf("[%s] %s\n", date_i18n('Y-m-d H:i:s'), $msg));
        fclose($tfp);
    }
