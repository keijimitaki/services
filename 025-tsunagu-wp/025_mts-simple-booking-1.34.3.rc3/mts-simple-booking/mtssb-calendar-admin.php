...
<?php if (!empty($booking)) : ?>
		...
		<table class="form-table booking-detail">
			...
			<?php if (!empty($booking['client']['birthday'])) : ?><tr>
				<th><?php _e('Birthday', $this->domain) ?></th>
				<td><?php $ymd = explode('-', $booking['client']['birthday']);
                    echo sprintf('%s年%s月%s日', ($ymd[0] == 0 ? ' ' : $ymd[0]), ($ymd[1] == 0 ? ' ' : sprintf('%02d', $ymd[1])), $ymd[2] == 0 ? ' ' : sprintf('%02d', $ymd[2])); ?></td>
			</tr><?php endif; ?>
