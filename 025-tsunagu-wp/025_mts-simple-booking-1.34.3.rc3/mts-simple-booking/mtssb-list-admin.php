...
if (!class_exists('MTS_WPDate')) {
    require_once('lib/MTSWPDate.php');
}
...
<?php
    ...
    {
        $ret = ['result' => false, 'message' => 'mtssb-list-admin ajax_dispatcher error.'];
    ...
    private function _out_mailform($booking, $article)
    {
        ...
        $option_name = $article['addition']->template;
        ...
        $oTemplate->getTemplate($option_name);
                ...
                switch ($column) {
                    ...
                    case 'birthday' :
                        if (!empty($client['birthday'])) {
                            echo sprintf("<tr><th>%s</th><td>%s</td></tr>\n",
                                '生年月日', date_format(date_create($client['birthday']), 'Y年n月j日'));
                        }
                        break;
                    case 'gender' :
                        $gender = '';
