<?php
/**
 * TsunaguMiyagi functions and definitions
 *
 ...
 */
function tsunagumiyagi_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on TsunaguMiyagi, use a find and replace
		* to change 'tsunagumiyagi' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'tsunagumiyagi', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'tsunagumiyagi' ),
		)
	);
...
add_action( 'after_setup_theme', 'tsunagumiyagi_setup' );
 ...
 */
function tsunagumiyagi_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'tsunagumiyagi_content_width', 640 );
}
add_action( 'after_setup_theme', 'tsunagumiyagi_content_width', 0 );
 ...
 */
function tsunagumiyagi_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'tsunagumiyagi' ),
			'id'            => 'sidebar-1',
...
//add_action( 'widgets_init', 'tsunagumiyagi_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function tsunagumiyagi_scripts() {
	//wp_enqueue_style( 'tsunagumiyagi-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_enqueue_style( 'tsunagumiyagi-style', get_theme_file_uri('css/style.css'), array(), _S_VERSION );
	wp_style_add_data( 'tsunagumiyagi-style', 'rtl', 'replace' );

	wp_enqueue_style( 'tsunagumiyagi-style-custom', get_theme_file_uri('css/tsunagumiyagi-style.css'), array(), _S_VERSION );

	wp_enqueue_script( 'tsunagumiyagi-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'tsunagumiyagi_scripts' );
...
// 自動実行用関数
function tsunagu_update_next_event_date() {
